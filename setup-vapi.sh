...
echo = Test  =
echo =========
curl -kv https://$CLUSTER.ampgw.sandbox.axwaytest.net:8443/hook/demo
#K8_INGRESS=$(kubectl describe -n kube-system service/traefik | grep "LoadBalancer Ingress" | awk "{print \$3}" | sed "s/,//")
#echo curl -kv --resolve $CLUSTER.ampgw.sandbox.axwaytest.net:443:$K8_INGRESS https://$CLUSTER.ampgw.sandbox.axwaytest.net/demo/hook

echo curl -kv https://$CLUSTER.ampgw.sandbox.axwaytest.net:8443/hook/demo
